...
export function buildScientificAnalysisSystemPrompt(categories: VisaCategory[]): string {
  return [
    ...
    'MANDATORY: Evaluate EVERY criterion id listed in the user checklist — one criterionEvaluations row per id.',
    'profileFacts.verifiedFacts must come only from the VERIFIED PROFILE FACT INVENTORY in the user message.',
    'profileEvidence[] must quote or paraphrase inventory facts (employer, role, publication title) — no invented metrics.',
    'evidenceScore: start from RULE-BASED BASE score; adjust max ±15 only with cited profileEvidence; otherwise keep within ±5.',
    'If no documentary evidence exists for a criterion, score ≤35 (unsupported) and state gap in gapSummary.',
    'Do NOT use template employers, products, sample candidates, or wrong-field paper titles.',
    'Schema:',
...
export function buildScientificAnalysisSystemPromptCompact(categories: VisaCategory[]): string {
  return [
    ...
    'gaps: include at least one row per criterion with evidenceScore<56; every gap MUST include criterionId (e.g. eb1a-6) from the checklist.',
    'roadmapActions: papers→suggestedTitles; patents/product→outline+domain; match candidate field from profile.',
    'Evaluate ALL criterion ids in user checklist. profileEvidence must quote fact inventory. ±15 from BASE only with evidence.',
    'Schema (abbreviated):',
...
export function buildScientificBenchmarkSystemPrompt(
  ...
  return [
    ...
    'currentScore must align with criterion evidence scores from profile and rule-based rubric in user message.',
    'quantityToBuild counts NEW assets to produce (papers, patents, products, articles, speaking slots, judging proof) — not files to upload.',
    'NO ASSUMPTIONS: scores and responsibilities must cite verified profile facts only.',
